...
namespace wi
{
	...
	public:
		...
		XMFLOAT2 GetTextureSize() const
		{
			if (!textureResource.IsValid())
				return std::move(XMFLOAT2(0.0f, 0.0f));
			auto& desc = textureResource.GetTexture().desc;
			XMFLOAT2 size((float)desc.width, (float)desc.height);
			return std::move(size);
		}

		XMFLOAT4 GetCollisionRectf() const
		{
			XMFLOAT4 rectf;
			rectf.x = params.pos.x - params.siz.x * params.pivot.x;
			rectf.y = params.pos.y - params.siz.y * params.pivot.y;
			rectf.z = params.siz.x;
			rectf.w = params.siz.y;
			return std::move(rectf);
		}
	};
}
