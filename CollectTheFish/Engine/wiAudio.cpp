...
namespace wi::audio
{
	...
	bool CreateSoundInstance(const Sound* sound, SoundInstance* instance, const int loop_count)
	{
		if (audio_internal == nullptr || !audio_internal->IsValid())
			return false;
        ...
        instanceinternal->buffer.LoopCount = loop_count;
        instance->SetLooped(loop_count > 0);
